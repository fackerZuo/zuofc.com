            ...
            <template v-for="(item, index) in formList[0]">
                <el-form-item :label="item.value" v-if="!item.show" :key="index">
                    <template v-if="item.key == 'avatar'">
                        ...
                        >
                            <img
                                v-if="avatar['url'] || form.avatar"
                                :src="avatar['url'] || form.avatar"
                            />
            ...
            <template v-for="(v, k, i) in formList[1]">
                <el-form-item :label="v" :key="i+22">
                    <template v-if="k == 'image'">
                        ...
                        >
                            <img
                                v-if="cover['url'] || form.cover['image']"
                                :src="cover['url'] || form.cover['image']"
                            />
    ...
    methods: {
        ...
        async onSubmit() {
            ...
            if (Object.keys(this.password).length > 0) {
                ...
                const result = await this.$http.post("/password", {
                ...
                if (this[item].url) {
                    total.push(
                        new Promise((resolve, reject) => {
                            this[item]["formData"].append(
                                "type",
                                this.form.upload_type
                            );

                            this.$http
                                .post("/upload", this[item].formData)
                                .then(res => {
            ...
            Promise.all(upload)
                ...
                .catch(err => {
                    this.fullscreenLoading = false;
                    this.$message.error(
                        "图片上传失败，请检查网络是否正常 or OSS信息是否填写正确!"
                    );
                });
        },
        emailChange(e) {
